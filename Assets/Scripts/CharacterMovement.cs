...
public class CharacterMovement : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (Input.GetMouseButtonDown(0) && !isAttacking && !isMagicAttacking && isGrounded)
        {
            ...
            DealDamageToMob(attackDamage); // ÑBÑçÑxÑçÑrÑpÑuÑ} Ñ}ÑuÑÑÑÄÑt Ñ~ÑpÑ~ÑuÑÉÑuÑ~ÑyÑë ÑÖÑÇÑÄÑ~Ñp ÑtÑ|Ñë ÑLÑKÑM
        }
    ...
    {
        RaycastHit hit;
        Ray ray = Camera.main.ScreenPointToRay(Input.mousePosition); // ÑLÑÖÑâ ÑyÑx Ñ{ÑpÑ}ÑuÑÇÑç, Ñ~ÑpÑÅÑÇÑpÑrÑ|ÑuÑ~Ñ~ÑçÑz ÑÑÑÖÑtÑp, Ñ{ÑÖÑtÑp ÑÉÑ}ÑÄÑÑÑÇÑyÑÑ Ñ}ÑçÑäÑé
        ...
        if (Physics.SphereCast(ray, attackRadius, out hit, 100f, mobLayer))
        {
            ...
            if (mobHealthBar != null)
            {
                ...
                Debug.Log("ÑPÑÄÑÅÑpÑ|Ñy ÑÅÑÄ Ñ}ÑÄÑqÑÖ! ÑNÑpÑ~ÑuÑÉÑuÑ~ ÑÖÑÇÑÄÑ~: " + damageAmount);
            }
        }
    }
}